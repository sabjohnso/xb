...
TEST_CASE("round-trip: simple element with attribute",
...
TEST_CASE("round-trip: element with multiple attributes",
          "[any_element_round_trip]") {
...
TEST_CASE("round-trip: namespaced elements", "[any_element_round_trip]") {
  std::string input =
      R"(<ns:root xmlns:ns="http://example.org"><ns:child/></ns:root>)";

  any_element parsed = parse_xml(input);
  CHECK(parsed.name().namespace_uri() == "http://example.org");
...
TEST_CASE("round-trip: multiple namespaces", "[any_element_round_trip]") {
  std::string input =
      R"(<a:root xmlns:a="http://a.example" xmlns:b="http://b.example">)"
      R"(<b:child a:id="1"/></a:root>)";

  any_element parsed = parse_xml(input);
  CHECK(parsed.name().namespace_uri() == "http://a.example");
