...
namespace {
  ...
  struct recorded_event {
    ...
    bool
    operator==(const recorded_event& other) const = default;
  };
      ...
      switch (reader.node_type()) {
        case xml_node_type::start_element:
          ...
          for (std::size_t i = 0; i < reader.attribute_count(); ++i) {
            writer.attribute(reader.attribute_name(i),
                             reader.attribute_value(i));
          }
          break;
...
TEST_CASE("round-trip: nested elements with attributes and text",
          "[xml_round_trip]") {
  std::string input =
...
TEST_CASE("round-trip: namespaced document", "[xml_round_trip]") {
  ...
  // reports expanded names (URI + local) but not the original prefix. The
  // writer uses local names without prefixes when no namespace_declaration was
  // made. Instead we compare at the event level: qnames with full URIs must
  // match.
  ...
  // only if we write without namespaces — which won't match the original
  // qnames.
  //
...
TEST_CASE("round-trip: namespace-aware replay", "[xml_round_trip]") {
  // Full namespace-aware round-trip: read, write with namespace declarations,
  // re-read.
  std::string input =
      ...
      switch (reader2.node_type()) {
        case xml_node_type::start_element:
          ...
          for (std::size_t i = 0; i < reader2.attribute_count(); ++i) {
            writer.attribute(reader2.attribute_name(i),
                             reader2.attribute_value(i));
          }
          break;
        case xml_node_type::end_element:
          writer.end_element();
