...
// --- Group ref with cardinality ---

TEST_CASE("group ref choice with unbounded generates vector of variant",
          "[codegen]") {
  schema s;
  s.set_target_namespace("http://example.com/test");
  ...
  // Complex type: sequence containing group ref with maxOccurs=unbounded
  std::vector<particle> particles;
  particles.emplace_back(
  ...
  REQUIRE(st != nullptr);
  REQUIRE(st->fields.size() == 1);
  // Should be a vector of the choice variant
  CHECK(st->fields[0].type.find("std::vector<std::variant<") !=
        std::string::npos);
  CHECK(st->fields[0].name == "choice");
}
...
TEST_CASE("group ref choice with optional generates optional variant",
  ...
  REQUIRE(st != nullptr);
  REQUIRE(st->fields.size() == 1);
  CHECK(st->fields[0].type.find("std::optional<std::variant<") !=
        std::string::npos);
  CHECK(st->fields[0].name == "choice");
}
