...
TEST_CASE("any_attribute construction with values", "[any_attribute]") {
  xb::any_attribute a{{"http://example.org", "attr"}, "hello"};
...
TEST_CASE("any_attribute equality", "[any_attribute]") {
  xb::any_attribute a{{"urn:ns", "x"}, "val"};
  xb::any_attribute b{{"urn:ns", "x"}, "val"};
  xb::any_attribute c{{"urn:ns", "x"}, "other"};
  xb::any_attribute d{{"urn:ns", "y"}, "val"};
...
TEST_CASE("any_attribute ordering", "[any_attribute]") {
  xb::any_attribute a{{"aaa", "x"}, "zzz"};
  xb::any_attribute b{{"bbb", "x"}, "aaa"};
  xb::any_attribute c{{"aaa", "x"}, "aaa"};
...
TEST_CASE("any_attribute usable as unordered_map key", "[any_attribute]") {
  ...
  xb::any_attribute key{{"urn:ns", "x"}, "val"};
  map[key] = 42;

  CHECK(map.at(key) == 42);
  CHECK(map.count({{"urn:ns", "x"}, "val"}) == 1);
  CHECK(map.count({{"urn:ns", "x"}, "other"}) == 0);
}
...
TEST_CASE("any_attribute stream output with empty namespace", "[any_attribute]") {
  xb::any_attribute a{{"", "local"}, "value"};
  std::ostringstream os;
  os << a;
