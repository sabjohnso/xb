...
TEST_CASE("qname copy semantics", "[qname]") {
  ...
  // After encapsulation, qname is immutable — copy independence is
  // guaranteed by value semantics.  We verify the copy holds the
  // expected values.
  CHECK(copied.namespace_uri() == "urn:example");
  CHECK(copied.local_name() == "element");
...
TEST_CASE("qname usable as unordered_map key", "[qname]") {
  ...
  CHECK(map.at(key) == 42);
  CHECK(map.count(xb::qname("urn:ns", "elem")) == 1);
  CHECK(map.count(xb::qname("urn:ns", "other")) == 0);
}
